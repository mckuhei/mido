...
def read_message(infile, status_byte, peek_data, delta, clip=False):
    ...
    if clip:
        data_bytes = [byte if byte < 255 else 255 for byte in data_bytes]
    else:
        for byte in data_bytes:
            if byte > 255:
                raise IOError('data byte must be in range 0..255')
...
def read_sysex(infile, delta, clip=False):
    ...
    if clip:
        data = [byte if byte < 255 else 255 for byte in data]
